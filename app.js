const BALLS={"Poké":{accent:"#EF4444",light:"#FCA5A5",bg:"#2e1a1a"},Friend:{accent:"#4CAF50",light:"#81C784",bg:"#3a2e6e"},Love:{accent:"#E91E8B",light:"#F48FB1",bg:"#2e1a24"},Lure:{accent:"#00BCD4",light:"#80DEEA",bg:"#1a2630"},Moon:{accent:"#7986CB",light:"#9FA8DA",bg:"#1a1c2e"},Dream:{accent:"#CE93D8",light:"#E1BEE7",bg:"#221a2e"},Beast:{accent:"#FF9800",light:"#FFCC80",bg:"#2e1f0a"},Fast:{accent:"#C6D82A",light:"#E6EE9C",bg:"#2a2060"},Heavy:{accent:"#90A4AE",light:"#B0BEC5",bg:"#1a1f22"},Level:{accent:"#FF5722",light:"#FFAB91",bg:"#3d1a40"},Safari:{accent:"#8BC34A",light:"#C5E1A5",bg:"#2a1f5a"},Sport:{accent:"#FFC107",light:"#FFE082",bg:"#2a2510"},Great:{accent:"#3B82F6",light:"#93C5FD",bg:"#1a2030"},Ultra:{accent:"#FBBF24",light:"#FDE68A",bg:"#2a1f00"},Premier:{accent:"#F87171",light:"#FECACA",bg:"#2a1a1a"},Luxury:{accent:"#D97706",light:"#FCD34D",bg:"#2a1e0a"},Repeat:{accent:"#EC4899",light:"#F9A8D4",bg:"#2a1020"},Timer:{accent:"#6B7280",light:"#D1D5DB",bg:"#1a1a1a"},Dusk:{accent:"#7C3AED",light:"#C4B5FD",bg:"#1a1030"},Heal:{accent:"#F472B6",light:"#FBCFE8",bg:"#2a1020"},Quick:{accent:"#34D399",light:"#A7F3D0",bg:"#0a2a1a"},Net:{accent:"#0EA5E9",light:"#7DD3FC",bg:"#0a1a2a"},Dive:{accent:"#1D4ED8",light:"#93C5FD",bg:"#0a0a2a"},Nest:{accent:"#65A30D",light:"#BEF264",bg:"#0a1a0a"}};
...
// ── Rotom Breed Onboarding State ─────────────────────────────────────────────
let bfState = { goals: new Set(), ball: null, ivs: new Set() };

// Balls available in breeding context (no Safari/Sport)
const BF_BALLS_APRI     = ['Moon','Love','Friend','Fast','Lure','Heavy','Level','Dream','Beast'];
const BF_BALLS_STANDARD = ['Poké','Great','Ultra','Premier','Luxury','Repeat','Timer','Dusk','Heal','Quick','Net','Dive','Nest'];

function bfRenderBallGrid() {
  const grid = document.getElementById('bf-ball-grid');
  if (!grid) return;
  const allBalls = [...BF_BALLS_APRI, ...BF_BALLS_STANDARD];
  grid.innerHTML = allBalls.map(name => {
    const bc = BALLS[name] || BALLS['Poké'];
    const active = bfState.ball === name ? 'active' : '';
    const imgSrc = ballUrl(name);
    return `<button class="bf-ball-chip ${active}" data-ball="${name}"
      style="--chip-accent:${bc.accent};--chip-light:${bc.light}"
      onclick="bfSelectBall('${name}')">
      <img src="${imgSrc}" width="20" height="20" style="image-rendering:pixelated" onerror="this.style.display='none'"/>
      ${name}
    </button>`;
  }).join('');
}

function bfSelectBall(name) {
  // Toggle — clicking same ball deselects
  bfState.ball = bfState.ball === name ? null : name;
  bfRenderBallGrid();
  bfUpdateBeginButton();
}

function bfToggleGoal(goal) {
  if (bfState.goals.has(goal)) {
    bfState.goals.delete(goal);
  } else {
    bfState.goals.add(goal);
  }
  // Update button styles
  const shinyBtn = document.getElementById('bf-goal-shiny');
  const statsBtn = document.getElementById('bf-goal-stats');
  if (bfState.goals.has('shiny')) {
    shinyBtn.style.borderColor = '#fde68a';
    shinyBtn.style.background  = '#2e2010';
    shinyBtn.style.boxShadow   = '0 0 12px #fde68a33';
  } else {
    shinyBtn.style.borderColor = '#5b469044';
    shinyBtn.style.background  = '#2b1f4e';
    shinyBtn.style.boxShadow   = 'none';
  }
  if (bfState.goals.has('stats')) {
    statsBtn.style.borderColor = '#c084fc';
    statsBtn.style.background  = '#3a1f5e';
    statsBtn.style.boxShadow   = '0 0 12px #c084fc33';
  } else {
    statsBtn.style.borderColor = '#5b469044';
    statsBtn.style.background  = '#2b1f4e';
    statsBtn.style.boxShadow   = 'none';
  }
  // Show/hide subsequent steps
  const hasGoal = bfState.goals.size > 0;
  document.getElementById('bf-step-ball').style.display    = hasGoal ? 'block' : 'none';
  document.getElementById('bf-step-stats').style.display   = bfState.goals.has('stats') ? 'block' : 'none';
  document.getElementById('bf-step-species').style.display = hasGoal ? 'block' : 'none';
  // Update Rotom bubble
  ...
  if (bfState.goals.has('shiny') && bfState.goals.has('stats')) {
    ...
    bubble.textContent = 'A powerhouse in the making-zzzt! Rotom is taking notes! Now — any ball preference? ⚡';
  } else {
    bubble.textContent = 'Bzzt! What are we cooking up today, Trainer? ⚡';
  }
  bfUpdateBeginButton();
}

function bfToggleIV(btn) {
  const iv = btn.dataset.iv;
  if (bfState.ivs.has(iv)) {
    bfState.ivs.delete(iv);
...
function bfSpeciesPreview() {
  const val = document.getElementById('bf-rotom-species')?.value?.trim();
  const img = document.getElementById('bf-rotom-species-img');
  if (!img) return;
  if (val) {
    const slug = val.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
    img.src = `https://img.pokemondb.net/sprites/sword-shield/icon/${slug}.png`;
...
function bfUpdateBeginButton() {
  const species  = document.getElementById('bf-rotom-species')?.value?.trim();
  const hasGoal  = bfState.goals.size > 0;
  const wrap     = document.getElementById('bf-begin-wrap');
  if (wrap) wrap.style.display = (hasGoal && species) ? 'block' : 'none';
}

function bfBegin() {
  const species = document.getElementById('bf-rotom-species')?.value?.trim() || '';
  // Pre-fill form from onboarding answers
  document.getElementById('bf-species').value = species;
  // Ball
  if (bfState.ball) {
    const sel = document.getElementById('bf-ball');
    if (sel) {
      for (let opt of sel.options) { if (opt.value === bfState.ball) { sel.value = bfState.ball; break; } }
    }
  }
  // IVs → build spread string
  if (bfState.ivs.size > 0) {
    const order = ['HP','Atk','Def','SpAtk','SpDef','Spe'];
    const spread = order.map(iv => bfState.ivs.has(iv) ? '31' : 'x').join('/');
    document.getElementById('bf-ivs').value = spread;
  }
  // Shiny checkbox
  if (bfState.goals.has('shiny')) {
    document.getElementById('bf-want-shiny').checked = true;
  }
  // Show/hide advanced section based on mode
  const isStats = bfState.goals.has('stats');
  const advSection = document.getElementById('bf-advanced-section');
  ...
  if (isStats) {
    ...
    document.getElementById('bf-advanced-toggle-label').textContent = '＋ Advanced options (nature, IVs, parents…)';
    advToggle._expanded = false;
  }
  // Mode badges
  document.getElementById('bf-mode-shiny-badge').style.display = bfState.goals.has('shiny') ? 'inline-flex' : 'none';
  document.getElementById('bf-mode-stats-badge').style.display = bfState.goals.has('stats') ? 'inline-flex' : 'none';
  // Switch screens
  document.getElementById('bf-rotom-screen').style.display = 'none';
  document.getElementById('bf-form-screen').style.display  = 'block';
  // Init autocomplete
  setTimeout(() => {
  ...
  updateBreedPreview();
  // Rotom reacts to species
  if (species) rotomSay('breedSpeciesReact', true);
}

function bfToggleAdvanced() {
  const btn      = document.getElementById('bf-advanced-toggle');
  const section  = document.getElementById('bf-advanced-section');
  const label    = document.getElementById('bf-advanced-toggle-label');
  btn._expanded  = !btn._expanded;
  section.style.display = btn._expanded ? 'block' : 'none';
  label.textContent     = btn._expanded
    ? '－ Hide advanced options'
    : '＋ Advanced options (nature, IVs, parents…)';
}

function bfBackToRotom() {
  document.getElementById('bf-rotom-screen').style.display = 'block';
  document.getElementById('bf-form-screen').style.display  = 'none';
}
...
function openNewBreedModal() {
  ...
  // Show Rotom onboarding screen, hide form
  document.getElementById('bf-rotom-screen').style.display = 'block';
  document.getElementById('bf-form-screen').style.display  = 'none';
  // Reset onboarding state
  bfState = { goals: new Set(), ball: null, ivs: new Set() };
  bfRenderBallGrid();
  // Reset step visibility
  document.getElementById('bf-step-ball').style.display    = 'none';
  document.getElementById('bf-step-stats').style.display   = 'none';
  document.getElementById('bf-step-species').style.display = 'none';
  document.getElementById('bf-begin-wrap').style.display   = 'none';
  // Reset goal buttons
  document.getElementById('bf-goal-shiny').className = '';
  document.getElementById('bf-goal-stats').className = '';
  document.getElementById('bf-goal-shiny').style.cssText = 'background:#2b1f4e;border:2px solid #5b469044;border-radius:14px;padding:16px 10px;cursor:pointer;transition:all .2s;text-align:center';
  document.getElementById('bf-goal-stats').style.cssText = 'background:#2b1f4e;border:2px solid #5b469044;border-radius:14px;padding:16px 10px;cursor:pointer;transition:all .2s;text-align:center';
  document.getElementById('bf-rotom-bubble').textContent = 'Bzzt! What are we cooking up today, Trainer? ⚡';
...
function openEditBreed(id) {
  ...
  // Skip Rotom screen for edits — go straight to form with all advanced visible
  document.getElementById('bf-rotom-screen').style.display  = 'none';
  document.getElementById('bf-form-screen').style.display   = 'block';
  document.getElementById('bf-advanced-section').style.display = 'block';
  document.getElementById('bf-advanced-toggle').style.display  = 'none';
  // Set mode badges from saved data
  document.getElementById('bf-mode-shiny-badge').style.display = p.wantShiny ? 'inline-flex' : 'none';
  document.getElementById('bf-mode-stats-badge').style.display = (p.targetIVs && p.targetIVs !== '') ? 'inline-flex' : 'none';
  showModal('breedModal');
  updateBreedPreview();
}
...
const ROTOM_QUIPS = {
  ...
  breedGoalShiny: [
    "Oooh, chasing sparkliezzz? Rotom can't WAIT to see it-zzzt! ⚡",
    "A shiny one! My circuits are tingling already, bucko-zzzt! ✨",
    "Bzzt! One gorgeous shiny coming right up — let'zzz get planning! ⚡",
  ],
  breedGoalStats: [
    "A powerhouse in the making-zzzt! Rotom is taking notezzz! ⚡",
    "Bzzt! Now we're cooking with electricity! Let'zzz nail those stats-zzzt!",
    "Competitive breeding! My circuitz are READY for this-zzzt! ⚡",
  ],
  breedGoalBoth: [
    "Shiny AND stats?! Bzzt-zzt! You really don't do things by halves, do you-zzzt! ⚡",
    "A shiny powerhouse! Rotom respectzzz the ambition-zzzt! ✨⚔️",
    "KZZZZRRT?! Best of both worlds! This is going to be a long one, bucko-zzzt! ⚡",
  ],
  breedSpeciesReact: [
    "Bzzt! Excellent choice-zzzt! Let'zzz get this Egg warmed up! ⚡",
    "Ooh, a fine pick! Rotom knowzzz a good Trainer's taste when I see it-zzzt!",
    "Zzzt! Now THAT is a Pokémon worth breeding for-zzzt! Let'zzz go! ⚡",
  ],
  idle_legend: [
